...
add_action( 'plugins_loaded', 'wsal_yoast_seo_extension_init_actions' );

/**
 * Check if plugin is being installed via a multisite child site, if so, show notice.
 */
function wsal_yoast_seo_extension_init_actions() {
 ...
 */
function wsal_yoast_seo_extension_plugin_deactivate() {
 ...
 */
function wsal_yoast_seo_extension_install_notice() {
...
add_action( 'admin_init', 'wsal_yoast_seo_extension_init_install_notice' );

function wsal_yoast_seo_extension_init_install_notice() {
	...
	if ( ! class_exists( 'WpSecurityAuditLog' ) && ! class_exists( 'WSAL_AlertManager' ) ) {
		...
		if ( get_option( 'wsal_forms_notice_dismissed' ) != true ) { // phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison -- this may be truthy and not explicitly bool
			...
			if ( is_multisite() && is_network_admin() ) {
				add_action( 'admin_notices', 'wsal_yoast_seo_extension_install_notice' );
				add_action( 'network_admin_notices', 'wsal_yoast_seo_extension_install_notice', 10, 1 );
			} elseif ( ! is_multisite() ) {
				add_action( 'admin_notices', 'wsal_yoast_seo_extension_install_notice' );
			}
 ...
 */
function wsal_yoast_seo_extension_scripts() {
	wp_enqueue_script(
...
add_action( 'admin_enqueue_scripts', 'wsal_yoast_seo_extension_scripts' );

/**
 * Update option if user clicks dismiss.
 */
function wsal_yoast_seo_extension_dismiss_notice() {
	update_option( 'wsal_forms_notice_dismissed', true );
}
add_action( 'wp_ajax_wsal_yoast_seo_extension_dismiss_notice', 'wsal_yoast_seo_extension_dismiss_notice' );

/**
* Hook into WSAL's action that runs before sensors get loaded.
*/
add_action( 'wsal_before_sensor_load', 'wsal_yoast_seo_extension_mu_plugin_add_custom_sensors_and_events_dirs' );
 ...
 */
function wsal_yoast_seo_extension_mu_plugin_add_custom_sensors_and_events_dirs( $sensor ) {
	add_filter( 'wsal_custom_sensors_classes_dirs', 'wsal_yoast_seo_extension_mu_plugin_custom_sensors_path' );
	add_filter( 'wsal_custom_alerts_dirs', 'wsal_yoast_seo_extension_mu_plugin_add_custom_events_path' );
	return $sensor;
}
 ...
 * @return array
 */
function wsal_yoast_seo_extension_mu_plugin_custom_sensors_path( $paths = array() ) {
 ...
 * @return array
 */
function wsal_yoast_seo_extension_mu_plugin_add_custom_events_path( $paths ) {
	$paths   = ( is_array( $paths ) ) ? $paths : array();
