...
if ( ! class_exists( '\WSAL\Plugin_Sensors\Yoast_SEO_Sensor' ) ) {
	 ...
	 * @subpackage Sensors
	 */
	if ( ! class_exists( 'Yoast_SEO_Sensor' ) ) {
		/**
		...
		class Yoast_SEO_Sensor {
			 ...
			 * @var array
			 */
			private static $post_seo_data = array(
				'_yoast_wpseo_title'                => '',
			 ...
			 */
			private static function retrieve_post_data() {
			 ...
			 */
			private static function set_post_seo_data() {
				$post_meta = get_post_meta( self::$post_id, '', true );
				...
				foreach ( $wanted_keys as $yoast_meta_key ) {
					self::$post_seo_data[ $yoast_meta_key ] = isset( $post_meta[ $yoast_meta_key ][0] ) ? $post_meta[ $yoast_meta_key ][0] : false;
				}
			}
				...
				if ( $old_schema !== $schema ) {
					...
					Alert_Manager::trigger_event(
						...
						array(
							...
							'PostDate'           => self::$post->post_date,
							'PostUrl'            => get_permalink( self::$post->ID ),
							'old_type'           => ( $old_schema ) ? self::$schema_labels[ $old_schema ] : esc_html__( 'Default', 'activity-log-wp-seo' ),
							'new_type'           => ( $schema ) ? self::$schema_labels[ $schema ] : esc_html__( 'Default', 'activity-log-wp-seo' ),
							$editor_link['name'] => $editor_link['value'],
						)
					);
				}
			...
			private static function yoast_setting_change_alert( $key, $old_value, $new_value ) {
				...
				switch ( $key ) {
					...
					case 'schema-page-type-attachment':
						...
						$alert_args['old_type']    = ( $alert_args['old'] ) ? self::$schema_labels[ $alert_args['old'] ] : esc_html__( 'Default', 'activity-log-wp-seo' );
						$alert_args['new_type']    = ( $alert_args['new'] ) ? self::$schema_labels[ $alert_args['new'] ] : esc_html__( 'Default', 'activity-log-wp-seo' );
						break;
